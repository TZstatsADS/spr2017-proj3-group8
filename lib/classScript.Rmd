...
# Test Features
 ...
 test_feature_hs <- sample(1:2000, 400)
 test_label_hs <- label[test_feature_hs]
 test_feature_hs <- feature_hs[test_feature_hs,]

test_features_hs <- read.csv("../output/test_hog_feature+sift.csv", header = TRUE)
test_feature_s <- read.csv("../output/sift_features/test_sift_features.csv", header = TRUE)
...
```{r}
rf_preds <- rf_test(fit_train=rf_model_hs, dat_test=test_feature_hs)
nn_preds <- nn_test(nn=nn_model_hs, test=test_feature_hs)
gbm_preds <- gbm_test(fit_train=gbm_model_hs, dat_test=test_feature_hs)

preds_hs <- rbind(rf_preds,nn_preds,gbm_preds)
preds_hs <- round(colMeans(preds),0)
```